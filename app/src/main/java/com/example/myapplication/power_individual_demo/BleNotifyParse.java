...
public class BleNotifyParse extends BaseBleMessage{
	...
	private boolean Comm_Handle(BaseActivity hrDK, byte[] notifyData, int dataLength) {
		...
		received = BaseBleMessage.byteArrHexToString(frame);
		byte cmd = frame[1];
		...
		if ((cmd & 0x80) == 0x80) {
			...
			if(req_cmd == BleCmd06_getData.mTheCmd) {
				BleCmd06_getData ctrlCmd = new BleCmd06_getData();
				ret = ctrlCmd.dealBleResponse(Data, dataLen);
			}else if (req_cmd == BleCmd09_getAllData.mTheCmd) {
				BleCmd09_getAllData ctrlCmd = new BleCmd09_getAllData();
				ret = ctrlCmd.dealBleResponse(Data, dataLen);
			}else {
				return false;
			}
